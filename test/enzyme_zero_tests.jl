...
@testset "Enzyme: zero-valued differentiable coefficients" begin
    ...
    cases = (
        ...
        ("atan constant", a -> cst(atan(CTPS(a, 1))), a -> 1/(1+a*a)),
        ("atan linear coefficient", a -> element(atan(CTPS(a, 1)), [1]), a -> -2a/(1+a*a)^2),
        ("atan quadratic coefficient", a -> element(atan(CTPS(a, 1)), [2]), a -> (3a*a-1)/(1+a*a)^3),
